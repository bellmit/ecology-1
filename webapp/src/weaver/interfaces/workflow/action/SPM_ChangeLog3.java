...
public class SPM_ChangeLog3 extends BaseBean
implements Action
{
public String execute(RequestInfo request)
{
  try
  {
    writeLog("供应商变更记录3--开始");
    RecordSet rs = new RecordSet();
    UUID uuid = UUID.randomUUID();
    int modeId=77; //建模id
    String billid = request.getRequestid();
    String sql="";
    sql="select * from uf_spm_blacklist where requestid="+billid;
    rs.executeSql(sql);
    if (rs.next()) {
      ...
      writeLog("供应商变更记录3--结束");
    }    
  }
  catch (Exception e) {
    writeLog("供应商变更记录3--出错" + e);
    return "0";
  }
  return "1";
 }
