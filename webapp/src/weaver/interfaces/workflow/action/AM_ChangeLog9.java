...
public class AM_ChangeLog9 extends BaseBean
implements Action
{
public String execute(RequestInfo request)
{
  try
  {
    ...
    String sql="";
    sql="select b.assetname,a.creator from 	uf_am_artclesreturn a,uf_am_artclesreturn_dt1 b where a.id=b.mainid and a.requestid="+billid;
    rs.executeSql(sql);
    while (rs.next()) {     
    	String  title="低值易耗品归还";//变更事项      	
      	...
      	+"dzyhpname,title,flowid,creator,createdate,uuid) values ("+modeId+",1,0,convert(varchar(10),getdate(),120),convert(varchar(8),getdate(),108),"
      ...
      writeLog("低值易耗品变更记录9--结束");
    }    
  }
  catch (Exception e) {
    writeLog("低值易耗品变更记录9--出错" + e);
    return "0";
  }
  return "1";
 }
