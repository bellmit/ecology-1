 ...
 * @version 1.0
 */
public class EASProjectContractAccount extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		try {
			...
			String sql = "select sqr,bm,htdf,khyh,khmc,yhzh,accountBankBrow,accountBankBranch,belongProCity from formtable_main_125 a inner join formtable_main_125_dt2 b on a.id=b.mainid where a.requestId='"
					+ requestid + "'";
			String sqr = ""; // 创建人
			String bm = ""; // 部门
			String htdf = ""; // 合同对方
			...
			while (rs1.next()) {
				...
				htdf = Util.null2String(rs1.getString("htdf"));
				...
				if (count.equals("0")) { // coun=0 则说明表中 没有该供应商 则新增
					...
					rs7.executeSql(sqlcity);
					if(rs7.next()){
						city=Util.null2String(rs7.getString("city"));
					}
					writeLog("sqlcity:"+sqlcity);
