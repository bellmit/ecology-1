...
public class SPM_Glcg3 extends BaseBean
  implements Action
{
  public String execute(RequestInfo request)
  {
    try
    {
      ...
      String sqr ="";
      String sqrbm ="";
      String sql="select id,supplier,(select xmmc from formtable_main_134 where id=formtable_main_136.xmmc) as xmmc,sqrq,sqr,sqrbm from formtable_main_136  where requestid="+billid;
      rs.executeSql(sql);
      if (rs.next()) {
          ...
          for (int i = 0; i < supplier.length; i++) {
           ...
           sql="select count(1) as flag from formtable_main_136_dt1  where mainid='"+ids+"' and (supplier='"+supplier[i]+"' or agentsupplier='"+supplier[i]+"')";
           rs1.executeSql(sql);
           if (rs1.next()) {
        ...
        if (rs2.next()){
          ...
          writeLog("管理采购流程生成供应商合作数据--成功");
       }      
    }
    catch (Exception e) {
      writeLog("管理采购流程生成供应商合作数据--出错" + e);
      return "0";
    }
    return "1";
  }
