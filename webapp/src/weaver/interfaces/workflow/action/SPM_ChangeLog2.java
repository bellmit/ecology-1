...
public class SPM_ChangeLog2 extends BaseBean
implements Action
{
public String execute(RequestInfo request)
{
  try
  {
    writeLog("供应商变更记录2--开始");
    RecordSet rs = new RecordSet();
    UUID uuid = UUID.randomUUID();
    int modeId=77; //建模id
    String billid = request.getRequestid();
    String sql="";
    sql="select * from uf_spm_supplierbg where requestid="+billid;
    rs.executeSql(sql);
    if (rs.next()) {
    	String  title="";//变更事项
      ...
      writeLog("供应商变更记录2--结束");
    }    
  }
  catch (Exception e) {
    writeLog("供应商变更记录2--出错" + e);
    return "0";
  }
  return "1";
 }
