...
public class CRM_IncomeOrder extends BaseBean implements Action {
	public String execute(RequestInfo request) {
		try {
			...
			// 更新购买产品--默认等于所属业务
			sql = "update uf_crm_orderinfo set product =(select name from uf_crm_businessinfo where id=uf_crm_orderinfo.business) where flowid="
					+ billid;
			rs1.executeSql(sql);

			// 更新开票状态，订单状态，付款期限
			sql = "update uf_crm_orderinfo set kpstatus='0',skstatus='0',status='0',paydate=convert(varchar(10),dateadd(ms,-3,DATEADD(mm, DATEDIFF(m,0,orderdate)+1, 0)),120) where flowid="
					+ billid;
			rs2.executeSql(sql);
			writeLog("开票申请更新订单数据--结束");

			// 更新客户业务卡片中的应收金额
			sql = "select a.business,b.customer,b.whetherShare,b.invoicemoney,b.currentmoney,b.startdate,b.enddate,"
					+ "year(b.startdate) as startyear,month(b.startdate) as startmonth,day(b.startdate) as startday,"
					+ "year(b.enddate) as endyear,month(b.enddate) as endmonth,"
					+ "(case when month(b.startdate)=month(b.enddate) then DATEDIFF(mm,b.startdate, b.enddate) else DATEDIFF(mm,b.startdate, b.enddate)+1 end) as totalmonth,"
					+ "a.createdate,b.uuid from uf_crm_invoice a,uf_crm_invoice_dt1 b where a.id=b.mainid  and a.requestid='"
					+ billid + "'";
			rs3.executeSql(sql);
			while (rs3.next()) {
				String business = Util.null2String(rs3.getString("business"));// 所属业务
				String customer = Util.null2String(rs3.getString("customer"));// 客户
				String whetherShare = Util.null2String(rs3
						.getString("whetherShare"));// 是否分摊
				...
				String orderdate = Util
						.null2String(rs3.getString("createdate"));// 订购日期
