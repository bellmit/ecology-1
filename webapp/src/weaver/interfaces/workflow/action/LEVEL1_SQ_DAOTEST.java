...
public class LEVEL1_SQ_DAOTEST extends BaseCronJob{
	...
	public void execute(){ 
		try {
			lg.writeLog("level1申请流程触发开始----"+syndate);
			create();
		} catch (Exception e) {
	...
	public void create(){
		...
		while(rds.next()){
			...
			String sqxknxbegin = Util.null2String(rds.getString("APPLY_START_TIME"));//申请许可年限---------
			...
			//String cpljdz = Util.null2String(rds.getString("PRODUCT_URL"));
			...
			//String ywbdynr = Util.null2String(rds.getString("RECEIVE_DEVICE_OTHER"));
			
			//String ywbdenr = Util.null2String(rds.getString("TRANSMISSION_MODE_OTHER"));
						
			//创建工作流开始
			try {
				lg.writeLog("level1申请创建流程");
//				syid,gsid,gsmc,zcdz,zzjgdm,frdb,
...
//				tcsj,pc,wife,dsjjdh,other,zx,wx,jyw,hlw,qt,qtgnhxx,sqxknr,wxzz, xzgly
	...
	public int createWF(String syid,String gsid,String gsmc,String zcdz,String zzjgdm,String frdb,
			...
			//,String ywbdynr,String ywbdenr,wxzz, xzgly
			){
		String newrequestid = "";
		try{
			String workflowid = "59";	
			String lcbt = "LEVEL-1 申请-"+syndate;// 流程标题   
			RequestService requestService = new RequestService();
			RequestInfo requestInfo = new RequestInfo();
			
			requestInfo.setWorkflowid(workflowid);//流程类型id
			requestInfo.setCreatorid("42");//创建人		
...
//7,gswz,glfzr,fzrdh,ywlxr,lxrdh,gscz,
			...
			//field = new Property();
			//field.setName("sqsxjsrq");
			//field.setValue(sqsxjsrq);
			//fields.add(field);
			...
			//field = new Property();
			//field.setName("cpljdz");
			//field.setValue(cpljdz);
			//fields.add(field);			
			if(mbkh !=""){
			field = new Property();
			field.setName("mbkh");
			...
			while(fjss.next()){
			    ...
			    int aa = sc.getContent(gsid,"42",NRBT);
			    if(aa != 0){
			    	docids = docids+","+aa;
			    }
