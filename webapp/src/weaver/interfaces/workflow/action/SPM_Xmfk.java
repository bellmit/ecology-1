...
public class SPM_Xmfk extends BaseBean
  implements Action
{
  public String execute(RequestInfo request)
  {
    try
    {
      ...
      String htids ="";
      String paytitle ="";
      String sql="select a.sqr,a.bm,b.id,a.htdf,c.htmc,a.htmc as htid,d.fkje,(select selectname from workflow_SelectItem where selectvalue=d.fkjh and fieldid='9464') as paytitle from formtable_main_126 a join uf_spm_supplier b on a.htdf=b.suppliername join uf_htkp c on a.htmc=c.id left join formtable_main_126_dt1 d on a.id=d.mainid where c.sqlx='0' and a.requestid="+billid;
      rs.executeSql(sql);
      while (rs.next()) {
          ...
          String  wyid =uuid.toString();//生成唯一的标识码         	
          	...
          	+"supplier,createdate,creator,dept,payflow,paycontent,money,paydate,paytitle,xmhtid,uuid) values ("+modeId+",1,0,convert(varchar(10),getdate(),120),convert(varchar(8),getdate(),108),"
          ...
          writeLog("项目付款流程生成供应商付款数据--成功");
       }      
    }
    catch (Exception e) {
      writeLog("项目付款流程生成供应商付款数据--出错" + e);
      return "0";
    }
    return "1";
  }
