...
public class SSEWorkflow_ImportEAS_272_action extends BaseBean implements Action {
    public String execute(RequestInfo request){
        ...
        try{
			...
			for(int i = 0; i< rows.length; i++){
				...
				for(int j=0; j<cells.length; j++){
					...
					if(cell.getName().equals("se")){
						...
						fplx = Util.null2String(cell.getValue());
					}else if(cell.getName().equals("fyrq")){
						fyrq = Util.null2String(cell.getValue());
					}
				}
				if(se > 0 && fplx.equals("0")){//如果有税额&发票类型为增票，则一行明细两个分录
					...
					for (int j = 0; j < 2; j++) {
						...
						vochers[entrySeq].setVoucherAbstract(lastname+fyrq+fylx);
			...
			vochers[entrySeq].setVoucherNumber(voucherNumber);
			vochers[entrySeq].setEntrySeq(entrySeq+1);
			vochers[entrySeq].setVoucherAbstract(lastname+"业务招待费");
			...
			if(result1.lastIndexOf("成功保存") >= 0){
	            ...
	            writeLog("业务活动费报销 流程向EAS抛凭证出错："+result1);
	            request.getRequestManager().setMessage("-1");
				request.getRequestManager().setMessagecontent("业务活动费报销 流程向EAS抛凭证出错："+result1);
				result = "0";
			}
			return result;
        }catch(Exception e){
            writeLog("业务活动费报销 流程向EAS抛凭证出错："+e);
            request.getRequestManager().setMessage("-1");
			request.getRequestManager().setMessagecontent("业务活动费报销 流程向EAS抛凭证出错："+e);
            return "0";
        }
    }
    //根据付款方式所选下拉框将值转换
