...
public class SSEWorkflow_ImportEAS_264_action extends BaseBean implements Action {
    public String execute(RequestInfo request){
        ...
        try{
			...
			String accountNumber = getaccountNumber(Util.null2String(mainTableDataMap.get("fykm")));//科目编码
			...
			writeLog("核算项目2-编码:"+asstActType21);
			writeLog("核算项目2-名称:"+asstActType22);
			//获得流程明细表数据-根据明细表数据做批量操作
			DetailTable dtltable =  request.getDetailTableInfo().getDetailTable(1);
			Row[] rows = dtltable.getRow();
			//构建数组 ，一个凭证一个数组
			String fplx = Util.null2String(mainTableDataMap.get("fplx"));//发票类型
			int length = 0;
			int entrySeq = 0;//分录行号，实际传递需+1
			double creditAmount = 0.00;//贷方金额
			double bhsje = 0.00;//不含税总金额
			double ses = 0.00;//税金额总
			...
			if(ses > 0 && fplx.equals("0") && !sfgdzcfk.equals("0")){//如果有税额&发票类型为增票&不是固定资产，则一行明细两个分录
				length = rows.length + 1 + 1;
			}else{
				length = 2;
			}
			WSWSVoucher[] vochers = new WSWSVoucher[length];
			if(ses > 0 && fplx.equals("0") && !sfgdzcfk.equals("0")){//如果有税额&发票类型为增票&不是固定资产，则一行明细两个分录
				creditAmount += ses;//累计贷方金额 不含税金额+税额
				for(int i = 0; i< rows.length; i++){
					Row row = rows[i];
					Cell[] cells = row.getCell();
					double se = 0.00;//税金额
			...
			vochers[entrySeq].setDescription(lcbh);
			writeLog("银行名称:"+getbankname(Util.null2String(mainTableDataMap.get("fkyh"))));
			writeLog("银行编码:"+getbankcode(Util.null2String(mainTableDataMap.get("fkyh"))));
			...
			if(result1.lastIndexOf("成功保存") >= 0){
	            ...
	            writeLog("付款审批流程new 流程向EAS抛凭证出错："+result1);
	            request.getRequestManager().setMessage("-1");
				request.getRequestManager().setMessagecontent("付款审批流程new 流程向EAS抛凭证出错："+result1);
				result = "0";
			}
			return result;
        }catch(Exception e){
            writeLog("付款审批流程new 流程向EAS抛凭证出错："+e);
            request.getRequestManager().setMessage("-1");
			request.getRequestManager().setMessagecontent("付款审批流程new 流程向EAS抛凭证出错："+e);
            return "0";
        }
    }
    //根据付款方式所选下拉框将值转换
