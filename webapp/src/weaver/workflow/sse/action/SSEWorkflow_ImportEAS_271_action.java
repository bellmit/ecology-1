...
public class SSEWorkflow_ImportEAS_271_action extends BaseBean implements Action {
    public String execute(RequestInfo request){
        ...
        try{
			...
			String meetingname = Util.null2String(mainTableDataMap.get("hymc"));//会议名称
			...
			for(int i = 0; i< rows.length; i++){
				...
				if(se > 0 && fplx.equals("0")){//如果有税额&发票类型为增票，则一行明细两个分录
					...
					for (int j = 0; j < 2; j++) {
						...
						vochers[entrySeq].setVoucherAbstract(lastname+meetingname);
			...
			vochers[entrySeq].setVoucherNumber(voucherNumber);
			vochers[entrySeq].setEntrySeq(entrySeq+1);
			vochers[entrySeq].setVoucherAbstract(lastname+meetingname+"报销");
			...
			if(result1.lastIndexOf("成功保存") >= 0){
	            ...
	            writeLog("会议费报销 流程向EAS抛凭证出错："+result1);
	            request.getRequestManager().setMessage("-1");
				request.getRequestManager().setMessagecontent("会议费报销 流程向EAS抛凭证出错："+result1);
				result = "0";
			}
			return result;
        }catch(Exception e){
            writeLog("会议费报销 流程向EAS抛凭证出错："+e);
            request.getRequestManager().setMessage("-1");
			request.getRequestManager().setMessagecontent("会议费报销 流程向EAS抛凭证出错："+e);
            return "0";
        }
    }
    //根据付款方式所选下拉框将值转换
