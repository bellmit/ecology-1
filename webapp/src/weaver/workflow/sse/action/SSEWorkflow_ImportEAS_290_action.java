...
public class SSEWorkflow_ImportEAS_290_action extends BaseBean implements Action {
...
writeLog("11155555555sdddd555："+periodNumber);			
			String lastname = rsutil.getHrmShowName(Util.null2String(mainTableDataMap.get("reimbursement")));//申请人姓名
			...
			int attaches = Util.getIntValue(Util.null2String(mainTableDataMap.get("accessoryAcount")), 0);//附件数量
			String voucherNumber = "";//凭证号
			String voucherType = convervoucherType(Util.null2String(mainTableDataMap.get("paytype")));//付款方式
			String lcbh = Util.null2String(mainTableDataMap.get("flownum"));//流程编号
			...
			for(int i = 0; i< rows.length; i++){
				...
				for(int j=0; j<cells.length; j++){
					Cell cell = cells[j];
					if(cell.getName().equals("InvoiceType")){
						invoicetype = Util.null2String(cell.getValue());
					}
				}
			...
			for(int i = 0; i< rows.length; i++){
				...
				Cell[] cells = row.getCell();
				String invoicetype = "";//发票类型
				double taxmoney = 0.00;//税金额
				double money = 0.00;//不含税金额
				double reimbursementmoney = 0.00;//报销金额
				...
				for(int j=0; j<cells.length; j++){
					Cell cell = cells[j];
					if(cell.getName().equals("TaxMoney")){
						taxmoney = Util.getDoubleValue(cell.getValue(),0);
					}else if(cell.getName().equals("syjsm")){
						voucherAbstract = Util.null2String(cell.getValue());
					}else if(cell.getName().equals("CostCode")){
						accountNumber = getaccountNumber(Util.null2String(cell.getValue()));
					}else if(cell.getName().equals("Money")){
						money = Util.getDoubleValue(cell.getValue(),0);
					}else if(cell.getName().equals("ReimbursementMoney")){
						reimbursementmoney = Util.getDoubleValue(cell.getValue(),0);
					}else if(cell.getName().equals("UndertakeDept")){
					...
					}else if(cell.getName().equals("InvoiceType")){
						invoicetype = Util.null2String(cell.getValue());
					}
				}
				if(taxmoney > 0 && invoicetype.equals("0")){//如果有税额&发票类型为增票，则一行明细两个分录
					creditAmount += money + taxmoney;//累计贷方金额 不含税金额+税额
					for (int j = 0; j < 2; j++) {
						int entryDC = 1;//分录行方向
					...
					creditAmount += money;//累计贷方金额 不含税金额
					vochers[entrySeq] = new WSWSVoucher();
					vochers[entrySeq].setCompanyNumber(companyNumber);
					vochers[entrySeq].setBookedDate(bookedDate);
					vochers[entrySeq].setBizDate(bizDate);
					...
					vochers[entrySeq].setOriginalAmount(money);
					vochers[entrySeq].setDebitAmount(money);
					vochers[entrySeq].setCreator(creator);
					vochers[entrySeq].setAsstActType1(asstActType1);
					vochers[entrySeq].setAsstActName1(asstActType12);
					vochers[entrySeq].setAsstActNumber1(asstActType11);
			...
			vochers[entrySeq].setVoucherNumber(voucherNumber);
			vochers[entrySeq].setEntrySeq(entrySeq+1);
			vochers[entrySeq].setVoucherAbstract(bookedDate+"文化建设费报销");
			vochers[entrySeq].setAccountNumber(convervoucherType1(Util.null2String(mainTableDataMap.get("paytype"))));
			vochers[entrySeq].setCurrencyNumber(currencyNumber);
			vochers[entrySeq].setLocalRate(localRate);
			vochers[entrySeq].setEntryDC(-1);
			...
			vochers[entrySeq].setAsstActName1(getbankname(Util.null2String(mainTableDataMap.get("paybank"))));
			vochers[entrySeq].setAsstActNumber1(getbankcode(Util.null2String(mainTableDataMap.get("paybank"))));
			vochers[entrySeq].setDescription(lcbh);
			...
			if(result1.lastIndexOf("成功保存") >= 0){
	            ...
	            writeLog("文化建设费报销 流程向EAS抛凭证出错："+result1);
	            request.getRequestManager().setMessage("-1");
				request.getRequestManager().setMessagecontent("文化建设费报销 流程向EAS抛凭证出错："+result1);
				result = "0";
			}
			return result;
        }catch(Exception e){
            writeLog("文化建设费报销 流程向EAS抛凭证出错："+e);
            request.getRequestManager().setMessage("-1");
			request.getRequestManager().setMessagecontent("文化建设费报销 流程向EAS抛凭证出错："+e);
            return "0";
        }
    }
    //根据付款方式所选下拉框将值转换
