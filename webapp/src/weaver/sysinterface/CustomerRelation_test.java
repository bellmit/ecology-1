...
public class CustomerRelation_test extends HttpServlet{
	...
	private void getAllinfo(User user) {
			...
			//userid="29";
			RecordSet rs = new RecordSet();
			String sql="select b.id,b.name,b.dsporder from uf_crm_custbusiness a,uf_crm_businessinfo b,Matrixtable_7 as c where a.business=b.id and b.id=c.business and  ("+userid+" in(select resourceid from hrmrolemembers where  roleid=161 or roleid=2) or "+userid+" in (select F1 from vfn_Splitstr(c.businessleader,',')) or "+userid+" in (select F1 from vfn_Splitstr(c.businessmanager,',')) or ("+userid+" in (select F1 from vfn_Splitstr(c.regionleader,',')) and a.regionalcenter is not null) or "+userid+" =a.accountmanager) group by b.id,b.name,b.dsporder  order by b.dsporder";	
			...
			while(rs.next()){
				...
				sql2="select a.id,a.name,b.linkid,b.linkdate," +
						"(select top 1 id from uf_crm_contract where customer=a.id and business='"+id+"' order by status,id) as htid," +
						"(select top 1 status from uf_crm_contract where customer=a.id and business='"+id+"' order by status,id) as htstatus," +
						"(select sum(contractmoney) as contractmoney from uf_crm_contract where customer=a.id and business='"+id+"' and status='0') as contractmoney," +
						...
						" where a.id=b.customer and b.business=c.business and b.business="+id+" " +
						" and ("+userid+" in(select resourceid from hrmrolemembers where  roleid=161 or roleid=2) " +
						" or "+userid+" in (select F1 from vfn_Splitstr(c.businessleader,',')) " +
						" or "+userid+" in (select F1 from vfn_Splitstr(c.businessmanager,',')) " +
						" or ("+userid+" in (select F1 from vfn_Splitstr(c.regionleader,',')) and b.regionalcenter is not null) " +
				...
				while(rs2.next()){
					...
					String contractmoney=Util.null2String(rs2.getString("contractmoney"));//合同金额					
					String invoicemoney=Util.null2String(rs2.getString("invoicemoney"));//开票金额
					String reallymoney=Util.null2String(rs2.getString("reallymoney"));//账款金额
					...
					String requirement=Util.null2String(rs2.getString("isrequirement"));//需求
							
					
					xcontent+="<tr>"+
					"<td class=\"second-child\">"+Serialnumber+"</td>"+
					"<td><a href=\"/formmode/view/AddFormMode.jsp?type=0&modeId=102&formId=-256&billid="+xid+"&opentype=0&customid=139&viewfrom=fromsearchlist&mainid=0\" target= \"_blank\" title=\""+xname+"\">"+xname+"</a></td>"+
					"<td class=\"second-child\"><a href=\"/formmode/view/AddFormMode.jsp?type=0&modeId=109&formId=-266&billid="+linkid+"\" target= \"_blank\">"+linkdate+"</a></td>"+
					"<td class=\"second-child\"><a href=\"/formmode/view/AddFormMode.jsp?type=0&modeId=112&formId=-268&billid="+htid+"\" target= \"_blank\">"+status+"</a></td>"+
					"<td class=\"second-child\">"+contractmoney+"</td>"+
					"<td class=\"second-child\">"+invoicemoney+"</td>"+
					"<td class=\"second-child\">"+reallymoney+"</td>"+
					"<td class=\"second-child\"><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=195&businessid="+id+"&customerid="+xid+"\" target= \"_blank\">"+qxstatus+"</a></td>"+											
					"<td class=\"second-child\"><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=149\" target= \"_blank\">"+audit+"</a></td>"+	
					"<td class=\"second-child\" style=\"border-right: none;\"><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=158\" target= \"_blank\">"+requirement+"</a></td>"+					
					"</tr>";
				}
				
				content+="<tr>"
							+"<td colspan=\"10\" style=\"background-color: rgb(248,248,248);border-right: none;color:#d44632;font-size:16px;\"><a href=\"/formmode/view/AddFormMode.jsp?type=0&modeId=104&formId=-257&billid="+id+"&customid=140\" target= \"_blank\">"+name+"</a></td>"											
						+"</tr>" +
						""+xcontent+""; 
	...
	private void getbusinessOptions(User user) {
			...
			rs.executeSql(sql);
			String Alloptions="<option value=\"0\"></option>"; 
			while(rs.next()){
				String id=Util.null2String(rs.getString("id"));
				String name=Util.null2String(rs.getString("name"));
	...
	private void searchBegin(User user) {
			...
			String sql="select DISTINCT c.name as busname,b.business "+
						"from uf_crm_customerinfo a,uf_crm_custbusiness b,uf_crm_businessinfo c,Matrixtable_7 as d "+ 
							...
							"and b.business=c.id  "+sqlwhere+" " +
							"and  ("+userid+" in(select resourceid from hrmrolemembers where  roleid=161 or roleid=2) or "+userid+" in (select F1 from vfn_Splitstr(d.businessleader,',')) or "+userid+" in (select F1 from vfn_Splitstr(d.businessmanager,',')) or ("+userid+" in (select F1 from vfn_Splitstr(d.regionleader,',')) and b.regionalcenter is not null) or "+userid+" =b.accountmanager) "+
			...
			while(rs.next()){
				...
				String sql2="";					 
				sql2="select a.id as custid,a.name,b.id,c.name as busname,b.business,b.linkid,b.linkdate," +
						"(select top 1 id from uf_crm_contract where customer=a.id and business='"+id+"' order by status,id) as htid," +
						"(select top 1 status from uf_crm_contract where customer=a.id and business='"+id+"' order by status,id) as htstatus," +
						"(select sum(contractmoney) as contractmoney from uf_crm_contract where customer=a.id and business='"+id+"' and status='0') as contractmoney," +
						"b.invoicemoney,b.reallymoney,b.qxid,b.qxstatus,b.isaudit,b.isrequirement " +						
						"from uf_crm_customerinfo a,uf_crm_custbusiness b,uf_crm_businessinfo c,Matrixtable_7 as d "+ 
						"where a.id=b.customer and b.business=c.id and c.id=d.business and b.business='"+id+"' "+sqlwhere+" "+  
						"and  ("+userid+" in(select resourceid from hrmrolemembers where  roleid=161 or roleid=2) " +
				...
				while(rs2.next()){
					Serialnumber++;
					String xid=Util.null2String(rs2.getString("custid"));
					String xname=Util.null2String(rs2.getString("name"));					
					String linkid=Util.null2String(rs2.getString("linkid"));//联系记录id
					String linkdate=Util.null2String(rs2.getString("linkdate"));//联系记录
					String htid=Util.null2String(rs2.getString("htid"));//合同id
	...
	private void getAllkhInfo(User user) {
			...
			selSql = "select top " + iNextNum +" id,name,creditcode,corporatdelegate,registeredcapital,founddate,address,createdate from uf_crm_customerinfo order by createdate desc";
			...
			while(rs.next()){
				...
				json.put("corporatdelegate", Util.null2String(rs.getString("corporatdelegate")));
				json.put("registeredcapital", Util.null2String(rs.getString("registeredcapital")));
	...
	private void searchbykh(User user) {
			...
			String selSql="";
			//取按时间倒序排序的数据的前iNextNum条倒序排序
			selSql = "select top " + iNextNum +" id,name,creditcode,corporatdelegate,registeredcapital,founddate,address,createdate from uf_crm_customerinfo "+sqlwhere+" order by createdate desc,id desc";
			//将上面得到的数据取按顺序排序的ipageset条数据顺序排序
			selSql = "select top " + ipageset +" t1.* from (" + selSql + ") t1 order by t1.createdate asc,t1.id asc";
			//将得到的数据倒序排序显示
			selSql = "select t2.* from (" + selSql + ") t2 order by t2.createdate desc,t2.id desc";
	...
	private void getkhglrole(User user) {
		RecordSet rs = new RecordSet();
		String userid = String.valueOf(user.getUID());
		String sql="select resourceid from hrmrolemembers where roleid=161 and resourceid='"+userid+"'";
		rs.executeSql(sql);
		String role="";
		String leaderRole="";
		...
		while(rs.next()){
			if(flag1==0){
				String businessleaderid=Util.null2String(rs.getString("businessleader"));
			...
			if(flag2==0){
				String custommanagerids=Util.null2String(rs.getString("custommanager"));
			...
			if(flag3==0){
				String businessmanagerid=Util.null2String(rs.getString("businessmanager"));
				if(userid.equals(businessmanagerid)){
					businessmanagerRole="业务经理";
					flag3=1;
				}
			}
			if(flag4==0){
				String regionleaderid=Util.null2String(rs.getString("regionleader"));
			...
			if(flag5==0){
				String regionmanagerid=Util.null2String(rs.getString("regionmanager"));
				if(userid.equals(regionmanagerid)){
					regionmanagerRole="区域中心客户经理";
					flag5=1;
				}
			}
			if(flag6==0){
				String customadminid=Util.null2String(rs.getString("customadmin"));
	...
	private void getywInfo(User user) {
			...
			String sql="select b.id,b.name,b.dsporder " +
					...
					"or "+userid+" in (select F1 from vfn_Splitstr(c.businessleader,',')) " +
					"or "+userid+" in (select F1 from vfn_Splitstr(c.businessmanager,',')) " +
					"or ("+userid+" in (select F1 from vfn_Splitstr(c.regionleader,',')) " +
					"and a.regionalcenter is not null) or "+userid+" =a.accountmanager) " +
			...
			while(rs.next()){
				...
				String sql2="select a.id,a.name " +
						...
						"or "+userid+" in (select F1 from vfn_Splitstr(c.businessmanager,',')) " +
						"or ("+userid+" in (select F1 from vfn_Splitstr(c.regionleader,',')) " +
						"and b.regionalcenter is not null) or "+userid+" =b.accountmanager)";
				rs2.executeSql(sql2);
				...
				if(flag){
					contentleft+="<div class=\"info-r\">"+
							...
							"<table style=\"border-collapse:separate; border-spacing:0px 20px;width:100%\">"+
								...
								"</tr>"+
								"<tr>"+
								"<td style=\"border-right: none;color:#0e90d2;font-size:16px;text-align:center;\">"+total+"</td>"+
	...
	private void getmenuInfo(User user) {
		...
		while(rs.next()){
			...
			if(flag7==0){
				String businessadminid=Util.null2String(rs.getString("businessadmin"));
		...
		if(role.indexOf("业务管理员")>-1){
			...
			content+="<div class=\"yewu-item\">"
				...
				+"<ul>"
					+"<li><a>|</a><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=175\" target=\"_blank\">我的客户</a></li>"
					+"<li><a>|</a><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=170\" target=\"_blank\">客户标签</a></li>"				
					+"<li><a>|</a><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=189\" target=\"_blank\">联系人</a></li>"
					+"<li><a>|</a><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=190\" target=\"_blank\">联系记录</a></li>"
					+"<li><a>|</a><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=191\" target=\"_blank\">走访计划</a></li>"									
				+"</ul>"
			+"</div>";
			content+="<div class=\"yewu-item\">"
				+"<div class=\"yewu-header\">合同 <i class=\"am-icon am-icon-chevron-right\"></i></div>"
				+"<ul>"
					+"<li><a>|</a><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=187\" target=\"_blank\">合同信息</a></li>"
					+"<li><a>|</a><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=188\" target=\"_blank\">格式合同</a></li>"
					+"<li><a>|</a><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=185\" target=\"_blank\">订单查询</a></li>"
					+"<li><a>|</a><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=180\" target=\"_blank\">许可公示查询</a></li>"
					+"<li><a>|</a><a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=186\" target=\"_blank\">开通权限查询</a></li>"
				+"</ul>"
			+"</div>";
			content+="<div class=\"yewu-item\">"
			...
			if(role.indexOf("市场部负责人")>-1){
				content+="<div class=\"yewu-item\">"
					...
					+"<ul>"
						+"<li><a>|</a><a href=\"/matrixmanage/pages/matrixtable.jsp?canedit=true&issystem=&matrixid=7&showtype=1\" target=\"_blank\">角色设置</a></li>"
	...
	private void getTodayRemind(User user) {		
		...
		int recordCount = 0;// 总记录数
		int pageno = Util.getIntValue(request.getParameter("pageno"), 1);// 当前页
		int pagesize = Util.getIntValue(request.getParameter("pagesize"), 5);// 每页显示数
		String sql = "SELECT count(1) FROM uf_crm_notice WHERE person='"
				+ userid + "' AND modedatacreatedate='" + syndate + "' AND status='0'";
		rs.executeSql(sql);
		if (rs.next()) {
			recordCount = rs.getInt(1);
		}
		...
		// 取按时间倒序排序的数据的前iNextNum条倒序排序
		selSql = "SELECT TOP " + 5
				+ "* FROM uf_crm_notice WHERE id NOT IN(SELECT TOP " + 0
				+ " id FROM uf_crm_notice WHERE person='" + userid
				+ "' AND  modedatacreatedate='" + syndate + "' AND status='0') AND  person='" + userid
				+ "' AND  modedatacreatedate='" + syndate + "' AND status='0'";
		String id = "";
		String title = "";
		String customer = "";
		String person = "";
		String noticedate = "";
		String business = "";
		String totalcount = "<a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=176\"  target= \"_blank\">"+recordCount+"</a>";
		String contents = "";
		int number = 1;
		JSONArray jsonArray = new JSONArray();
		...
		while (rs.next()) {
			...
			person = Util.null2String(rs.getString("person"));
			noticedate = Util.null2String(rs.getString("noticedate"));
			...
			contents += "<tr>"
					...
					+ ". "
					+ "</td>"
					+ "<td align=\"left\" width=\"450px\" ><a href=\"/formmode/view/AddFormMode.jsp?type=0&modeId=131&formId=-316&billid="+id+"&opentype=0&customid=176&viewfrom=fromsearchlist&mainid=0\"  target= \"_blank\" >"
					+ title
					+ "</a></td>"
					...
					+ business + "&customid=140\" target= \"_blank\">"
					+ businessname + "</a></td>"
					...
					+ "</tr>";
			number++;
            sql="update uf_crm_notice set status='1' where id='"+id+"'";
	...
	public void getUser(User user){
		...
		String sqlUser = " select hrs.id,'1' as rolelevel from hrmrolemembers hrme left join hrmroles hrs on hrme.roleid = hrs.id " +
				" where hrs.id = 150  and hrme.resourceid = '"+userid+"'"+ //公司领导角色
				" union all "+
				" select hrs.id,'2' as rolelevel from hrmrolemembers hrme left join hrmroles hrs on hrme.roleid = hrs.id " +
				" where hrs.id = 163  and hrme.resourceid = '"+userid+"'"+ //客户管理员角色
				" union all "+
				" select id,'3' as rolelevel from hrmresource where departmentid =3 and id = '"+userid+"'"+ //市场部人员;
				" union all "+
