...
public class PayList_CheckFirstRemove extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if ("getCheckFirstRemove".equals(action)) { // 处理接口
				log_01.writeLog("银企直连开始：_____01_____");
				getCheckFirstRemove(user);
	 ...
	 * 银企直连-处理接口
	 */
	private void getCheckFirstRemove(User user) {
		...
		if (userid.equals(firstTrial)) {
			...
			if (status.equals("1") && showHide.equals("0")) {
				SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
				String FirstRemoveDateNew = dateFormat.format(new Date());

				RecordSet rs = new RecordSet();
				String sql = "update uf_payList set showOrHide='1',firstRemoveDate='"
						+ FirstRemoveDateNew + "' where id='" + id + "'";
