...
public class CustomerRelationtest extends HttpServlet {
...
*/
	...
	private void getkhglrole(User user) {
		...
		while (rs.next()) {
			...
			if (flag3 == 0) {
				String businessmanagerid = Util.null2String(rs
			...
			if (flag5 == 0) {
				String regionmanagerid = Util.null2String(rs
	...
	private void getTodayRemind(User user) {
		...
		String sql = "SELECT count(1) FROM uf_crm_notice WHERE person='"
				+ userid + "' AND modedatacreatedate='" + syndate
				+ "' AND status='0'";
		rs.executeSql(sql);
		if (rs.next()) {
			recordCount = rs.getInt(1);
		...
		selSql = "SELECT TOP " + 5
				...
				+ "' AND  modedatacreatedate='" + syndate
				+ "' AND status='0') AND  person='" + userid
		...
		String noticedate = "";
		String business = "";
		String totalcount = "<a href=\"/formmode/search/CustomSearchBySimple.jsp?customid=176\"  target= \"_blank\">"
				+ recordCount + "</a>";
		String contents = "";
		int number = 1;
		JSONArray jsonArray = new JSONArray();
		...
		while (rs.next()) {
			...
			contents += "<tr>"
					...
					+ ". "
					+ "</td>"
					+ "<td align=\"left\" width=\"450px\" ><a href=\"/formmode/view/AddFormMode.jsp?type=0&modeId=131&formId=-316&billid="
					+ id
					+ "&opentype=0&customid=176&viewfrom=fromsearchlist&mainid=0\"  target= \"_blank\" >"
					+ title
					+ "</a></td>"
		...
		while (rs.next()) {
			...
			while (rs2.next()) {
				...
				jsonobject.put("invoicemoney",
						Util.null2String(rs2.getString("invoicemoney")));// 开票金额
				jsonobject.put("reallymoney",
						Util.null2String(rs2.getString("reallymoney")));// 账款金额
