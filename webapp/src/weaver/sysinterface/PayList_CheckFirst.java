...
public class PayList_CheckFirst extends HttpServlet {
...
log_01.writeLog("初审处理开始：_____01_____");
				getDispose(user);
log_01.writeLog("初审处理结束：_____02_____");				
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
...
BaseBean log_03 = new BaseBean();
		...
		if(userid.equals(firstTrial)){
			log_03.writeLog("初审处理开始：_____02_____操作数据ID"+id);
			RecordSet rsd=new RecordSet();
			rsd.executeSql("select dealstatus,showOrHide from uf_payList where id='"+id+"'");
			String status="";
			String showHide="";
			if(rsd.next()){
			...
			if(status.equals("0")&&showHide.equals("0")){
				...
				String sql="update uf_payList set dealstatus='2',firstDate='"+disposeTime+"' where id='"+id+"'";
	log_03.writeLog("初审处理开始：_____03_____updateSQL"+sql);
				rs.executeSql(sql);
				try {
					response.setContentType("application/json; charset=utf-8");  
				...
				log_03.writeLog("初审处理开始：_____05_____操作人或状态错误"+userid+"_____"+status);
				response.setContentType("application/json; charset=utf-8");  
				try {
...
log_03.writeLog("初审处理开始：_____01_____操作人误"+userid);
